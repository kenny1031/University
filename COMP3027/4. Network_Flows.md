...
$$
\underset{(A,B)}{\text{argmin}}\text{ cap}(A,B).
$$
...
## 20. Integrality Lemma
Assume all initial capacities are integers.

**Lemma.** At every intermediate stage of Ford-Fulkerson:
* all flow values $f(e)$ are integers;
* all residual capacities are integers.

*Proof by induction.* 

Base case: Initially, $f(e)=0$ for all edges, so all flows and residual capacities are integers.

Induction step: Assume all residual capacities are integers before an augmentation. The bottleneck value

$$
b=\min_{e\in P}c_f(e)
$$

is the minimum of positive integers, so $b$ is a positive integer.

Augmenting by $b$ adds or subtracts an integer from existing integer flows, so new flows remain integers.

Therefore new residual capacities also remain integers.

## 21. Integrality Theorem
If all capacities are integers, then there exists a max flow $f$ such that every edge flow $f(e)$ is an integer.

This theorem is essential for applications like bipartite matching and edge-disjoint pathsm where we need 0/1 flows to correspond to discrete choices.

## 22. Ford-Fulkerson Running Time
Let $F=v(f_{\max})$ be the value of the maximum flow.

If capacities are integers, each augmentation increases the flow value by at least 1. Therefore Ford-Fukerson makes at most $F$ augmentations.

If each augmenting path can be found in $O(m+n)$ time using BFS or DFS, then Ford-Fulkerson runs in $O((m+b)F)$. Often written as $O(mF)$ when $m\ge n-1$.

## 23. Is $O(mF)$ Polynomial?
Not necessarily. The input size depends on $m$, $n$, and $\log{D}$, where $D$ is the maximum capacity.

If $F$ is very large, $O(mF)$ can be exponential in the input length because capacities are represented in binary using only $\log{D}$ bits.

So generic Ford-Fulkerson is **pseudo-polynomial**, not strongly polynomial.

## 24. Choosing Good Augmenting Paths
Different choices of augmenting paths give different algorithms.

### 24.1 Generic Ford-Fulkerson
Choose any augmenting path.
* Iterations: $O(F)$
* Total time roughly: $O(mF)$

### 24.2 Edmonds-Karp #1: Maximum Bottleneck Path
Choose the augmenting path with largest bottleneck capacity.

Claim: If remaining max flow in residual graph is $F'$, then there exists an augmenting path with bottleneck at least $F'/m$.

Reason: If all s-t paths had bottleneck below $F'/m$, deleting edges with capacity below $F'/m$ would disconnect $s$ from $t$, producing a cut of capacity less than $F'$, contradicting max-flow min-cut.

Then each iteration sends at least $1/m$ of the remaining flow, so remaining flow shrinks by factor:

$$
1-\frac{1}{m}.
$$

After $O(m\log{F})$ iterations, remaining flow drops below 1. With integer capacities, that means no remaining positive flow is possible.

It can be proven using the inequality

$$
\bigg(1-\frac{1}{a}\bigg)^a<\frac{1}{e}.
$$

Thus Edmonds-Karp #1 makes $O(m\log{F})$ iterations.

If finding a maximum bottleneck path costs $O(m)$ or $O(m\log{n})$ depending on implementation, the resulting max-flow running time can be summarised as $O(m^2\log{F})$.

### 24.3 Capacity Scaling
Ford-Fulkerson can be improved via capacity scaling and choosing sufficiently large bottleneck paths.

The high-level idea:
* start with a large threshold $\Delta$,
* only use residual edges with capacity at least $\Delta$,
* reduce $\Delta$ gradually.

This avoids wasting many iterations on tiny augmentations.

### Edmonds-Karp #2: Shortest Augmenting Path
Choose an augmenting path with the fewest number of edges, usually by BFS in the residual graph.

The running time $O(nm^2)$ is polynomial in graph size and independent of $F$.

### 24.5 Orlin's Algorithm
An algorithm finding max flow in $O(nm)$ time, due to Orlin 2012.

However, the course focuses on reductions and the Ford-Fulkerson framework.

# Part VI. Reductions to Max Flow
## 25. What is a Reduction?
A reduction from Problem $X$ to Problem $Y$ means:
1. Translate an instance $I$ of $X$ into an instance of $f(I)$ of $Y$.
2. Solve $f(I)$ using an algorithm for $Y$.
3. Translate the solution of $f(I)$ back into a solution for $I$.

For reductions to max flow:
```text
Problem X -> Flow network G(I) -> Max flow -> Solution to X
```

![](figures/reductions1.png)
![](figures/reductions2.png)

## 26. How to Prove a Max Flow Reduction Correct
For optimisation problems, usually prove:

$$
\text{optimal value of original problem}=\text{max flow value}.
$$

We prove two directions:

### Direction 1: Original solution $\implies$ Flow
Given a feasible solution to the original problem of size/value $k$, construct a valid flow of value $k$.

Usually use:
* original constraints imply flow conservation;
* original constraints imply capacity constraints;
* solution size equals flow value.

### Direction 2: Flow $\implies$ Original solution
Given an integral flow of value $k$, construct a feasible original solution of size/value $k$. 

Usually use:
* flow integrality to avoid fractional choices;
* capacity constraints to enforce original constraints;
* flow value lemma to show size/value.

# Part VII. Bipartite Matching
## 27. Matching
...
a **matching** is a set of edges

$$
M\subseteq E,
$$

such that each vertex appears in at most one edge of $M$.

The **maximum matching problem** asks for a matching of maximum cardinality.

## 28. Bipartite Matching
A graph is bipartite if $V=L\cup R$, and every edge goes between $L$ and $R$.

The **bipartite matching problem**: Given a bipartite graph $G=(L\cup R, E)$, find a maximum-cardinality matching.

This models assignment problems:
* jobs to machines;
* students to projects;
* ads to impressions;
* applicants to positions.

## 29. Reduction from Bipartite Matching to Max Flow
Given bipartite graph $G=(L\cup R,E)$, construct a directed flow network $G'$:
1. Add source $s$.
2. Add sink $t$.
3. For each $u\in L$, add edge $(s,u)$ with capacity 1.
4. For each original edge $(u,v)\in E$ with $u\in L$, $v\in R$, add directed edge $(u,v)$ with capacity 1.
5. For each $v\in R$, add edge $(v,t)$ with capacity 1.

Then compute max flow in $G'$.

Extract matching

$$
M=\{(u,v)\in L\times R:f(u,v)=1\}.
$$
